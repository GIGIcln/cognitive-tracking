...
export default function AttendanceTab({ sessionId, players }: AttendanceTabProps) {
  ...
  useEffect(() => {
    ...
    getAttendance(sessionId)
      .then((res) => {
        ...
        for (const r of (res.data as { player_id: string; status: string }[])) map[r.player_id] = r.status
        setAttendance(map)
      })
      .catch(() => {})
  ...
  const setStatus = useCallback((playerId: string, status: string) => {
