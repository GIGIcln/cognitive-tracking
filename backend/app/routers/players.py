...
@router.get("/{player_id}/attendance", response_model=list[PlayerAttendanceItemResponse])
async def get_player_attendance(
    ...
    if scope is not None and await svc.get_with_group(player_id, allowed_group_ids=scope) is None:
        raise HTTPException(status_code=404, detail="Giocatore non trovato")
    return await AttendanceService(db).get_player_attendance(player_id, allowed_group_ids=scope)
