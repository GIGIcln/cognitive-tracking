...
def test_coach_cannot_create_group_returns_403(client):
    h = _token(client, _COACH_LOGIN)
    res = client.post("/api/groups", headers=h, json={
        "name": "Test", "category": "Agonistica", "birth_year": 2010, "level": "A"
    })
    assert res.status_code == 403


def test_responsabile_cannot_create_group_returns_403(client):
    h = _token(client, _RESP_LOGIN)
    res = client.post("/api/groups", headers=h, json={
        "name": "Test", "category": "Agonistica", "birth_year": 2010, "level": "A"
    })
    assert res.status_code == 403
...
def test_coach_cannot_create_player_returns_403(client):
    h = _token(client, _COACH_LOGIN)
    res = client.post("/api/players", headers=h, json={
        "first_name": "X", "last_name": "Y", "birth_year": 2010
    })
    assert res.status_code == 403


def test_coach_cannot_assign_player_returns_403(client):
    import uuid
    h = _token(client, _COACH_LOGIN)
    res = client.post(f"/api/players/{uuid.uuid4()}/assign", headers=h,
                      json={"group_id": str(uuid.uuid4())})
    assert res.status_code == 403


def test_coach_cannot_delete_session_returns_403(seeded):
    """Allenatore non può eliminare sessioni (solo admin)."""
    c, admin_h = seeded["client"], seeded["headers"]
    gid = seeded["group_id"]

    # Admin crea una sessione
    sess = c.post("/api/sessions", headers=admin_h, json={
        "group_id": gid,
    ...
    # Coach prova a eliminarla
    coach_h = {"Authorization": "Bearer " + c.post(
        "/api/auth/login", json=_COACH_LOGIN
    ).json()["access_token"]}
    res = c.delete(f"/api/sessions/{session_id}", headers=coach_h)
    assert res.status_code == 403


# ── Data scoping: allenatore vede solo i propri gruppi ───────────────────────

def test_coach_sees_only_own_group(seeded):
    """L'allenatore (coach) ha il gruppo seeded assegnato → lo vede nella lista."""
    c = seeded["client"]
    coach_h = {"Authorization": "Bearer " + c.post(
        "/api/auth/login", json=_COACH_LOGIN
    ).json()["access_token"]}
    res = c.get("/api/groups", headers=coach_h)
...
def test_responsabile_sees_all_groups(seeded):
    """Il responsabile tecnico vede tutti i gruppi senza restrizioni."""
    c = seeded["client"]
    resp_h = {"Authorization": "Bearer " + c.post(
        "/api/auth/login", json=_RESP_LOGIN
    ).json()["access_token"]}
    res = c.get("/api/groups", headers=resp_h)
    assert res.status_code == 200
    assert len(res.json()) >= 1


def test_coach_cannot_access_other_group_returns_403(seeded):
    ...
    res = c.get(f"/api/groups/{uuid.uuid4()}", headers=coach_h)
    assert res.status_code == 403


def test_coach_can_create_session_for_own_group(seeded):
    """L'allenatore può creare sessioni per il proprio gruppo."""
    c = seeded["client"]
    coach_h = {"Authorization": "Bearer " + c.post(
        "/api/auth/login", json=_COACH_LOGIN
    ).json()["access_token"]}
    res = c.post("/api/sessions", headers=coach_h, json={
...
def test_responsabile_cannot_create_session_returns_403(seeded):
    """Il responsabile tecnico non può scrivere sessioni."""
    c = seeded["client"]
    resp_h = {"Authorization": "Bearer " + c.post(
        "/api/auth/login", json=_RESP_LOGIN
    ).json()["access_token"]}
    res = c.post("/api/sessions", headers=resp_h, json={
        "group_id": seeded["group_id"],
        "session_date": "2025-12-01",
        "session_type": "Allenamento",
        "duration_min": 75,
